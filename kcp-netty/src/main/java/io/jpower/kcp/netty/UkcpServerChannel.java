...
public final class UkcpServerChannel extends AbstractNioMessageChannel implements ServerChannel, Runnable {
    ...
    private AtomicInteger CONV_INDEX = new AtomicInteger();
            ...
            try {
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    if (closeWaitKcp != null) {
                        ...
                        try {
                            // enhance
                            if (childCh.ukcp().getConv() == 0) {
                                log.debug("conv is zero");
                                if (byteBuf.readableBytes() == 4) {
                                    int conv = CONV_INDEX.incrementAndGet();
                                    childCh.ukcp().setConv(conv);
                                    log.debug("{}  set conv {} -> {}", childCh.id(), childCh.ukcp().getConv(), conv);
                                    ByteBuf buffer = alloc().buffer(4);
                                    buffer.writeIntLE(conv);
                                    output.out(buffer, childCh.ukcp().getKcp());
                                } else {
                                    log.debug(" readableBytes {}", byteBuf.readableBytes());
                                }
                                // enhance end
                            } else {
                                if (byteBuf.readableBytes() != 4) {
                                    log.debug(" update data:{}", ByteBufUtil.getBytes(byteBuf));
                                    try {
                                        childCh.kcpInput(byteBuf);
                                    } catch (Exception e) {
                                        log.error("{}  conv: {}", childCh.id(), childCh.ukcp().getConv(), e);
                                    }
                                } else {
                                    ByteBuf buffer = alloc().buffer(4);
                                    buffer.writeIntLE(childCh.ukcp().getConv());
                                    output.out(buffer, childCh.ukcp().getKcp());
                                    log.debug("push conv client need setConv");
                                }
                            }
